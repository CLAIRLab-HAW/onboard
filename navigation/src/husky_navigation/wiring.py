...
def scan_topic() -> str:
    ...
    AMCL und slam_toolbox lesen AUSSCHLIESSLICH LaserScan -- eine
    PointCloud2 koennen beide nicht verarbeiten.  Dieser Knoten ist deshalb
    kein Komfort, sondern Voraussetzung.
    """
...
def cmd_vel_topic() -> str:
    ...
    Das ist der Eingang 'external' des twist_mux (Prioritaet 1, die
    niedrigste) -- Joystick, RC und interaktiver Marker uebersteuern Nav2
    also jederzeit.  Typ ist geometry_msgs/TwistStamped, nicht Twist.
    """
...
def pointcloud_to_laserscan_params() -> dict:
    ...
    Das Hoehenband ist RELATIV ZU base_link (target_frame) und muss die
    Fahrebene enthalten -- ein Band ueber oder unter ihr liefert lauter
    `inf` und sieht wie ein kaputter Treiber aus.
    """
    return {
        "target_frame": BASE_FRAME,
        "transform_tolerance": 0.05,
        # Band um die Fahrebene: alles zwischen 10 cm unter und 50 cm ueber
        # base_link.  base_link liegt 13,228 cm ueber dem Boden (das URDF
        # setzt base_footprint mit z=-0.13228 darunter), das Band beginnt
        # also knapp ueber dem Boden.
        "min_height": -0.10,
        "max_height": 0.50,
