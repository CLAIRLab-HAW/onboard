...
test_platform_mock_e2e.py and test_navigation_e2e.py therefore land on different workers and drive off at the same time.
...
defined in this package serialises this package and NOTHING in ``apps/robot-mcp``, which drives the same base.  The
fixture below is only the local name for it.
"""
...
def exclusive_base():
    ...
    Every test that sends cmd_vel or a navigation goal MUST request this fixture -- and so must every test that asserts
    the base did NOT move, because a foreign drive makes that assertion fail for the wrong reason.
    """
