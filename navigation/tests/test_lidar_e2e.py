...
def replay():
    ...
    _exec(f"source ros-env; nohup ros2 launch "
...
def test_the_driver_reads_from_the_recording(replay):
    ...
    assert "Receive Packets From : Pcap" in log, (
        f"Der Treiber liest nicht aus der Aufnahme. Log:\n{log[-2000:]}")


def test_the_point_cloud_arrives(replay):
    out = _exec("source ros-env; timeout 15 ros2 topic hz "
...
def test_the_cloud_carries_the_canonical_frame(replay):
    out = _exec("source ros-env; timeout 10 ros2 topic echo "
...
def test_amcl_publishes_the_map_to_odom_transform(replay):
    ...
    _exec("nohup nav localization:=amcl > /tmp/nav-amcl.log 2>&1 & sleep 35; echo ok",
          timeout=120)
    out = _exec("source ros-env; timeout 10 ros2 run tf2_ros tf2_echo map odom "
